...
router.post('/topup', async (req, res) => {
  ...
  if (process.env.FLUTTERWAVE_LIVE === 'true') {
    return res.status(501).json({ error: 'Live payment not configured. Set FLUTTERWAVE_LIVE=true with valid keys.' });
  }

  const transactionRef = `AVTPP-${uuidv4().slice(0, 8).toUpperCase()}`;
  const conn = await (await getPool()).getConnection();
  try {
    // ── MOCK MOBILE-MONEY SIMULATION ──
    await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 1000)); // 1-2s processing delay
    const isSuccess = Math.random() < 0.95; // 95% success

    // Wrap the whole record-and-credit sequence in one transaction with a row
    // lock, so concurrent top-ups can't interleave (no lost updates / partial writes).
    await conn.beginTransaction();
    try {
      const [accounts] = await conn.execute('SELECT * FROM Accounts WHERE UserID = ? FOR UPDATE', [req.user.userId]);
      if (accounts.length === 0) {
        await conn.rollback();
        return res.status(404).json({ error: 'Wallet not found.' });
      }
      ...
      if (isSuccess) {
        await conn.execute(
          ...
          [account.AccountID, topUpAmount, provider, phoneNumber, transactionRef, 'Completed']
        );
        await conn.execute(
          'UPDATE Accounts SET Balance = Balance + ? WHERE AccountID = ?',
          [topUpAmount, account.AccountID]
        );
        ...
        return res.json({
          ...
          transactionRef, amount: topUpAmount, provider, newBalance,
          currency: 'ZMW', status: 'Completed', mode: 'simulation'
        });
      } else {
        await conn.execute(
          'INSERT INTO TopUps (AccountID, Amount, Provider, PhoneNumber, TransactionRef, Status) VALUES (?, ?, ?, ?, ?, ?)',
          [account.AccountID, topUpAmount, provider, phoneNumber, transactionRef, 'Failed']
        );
        await conn.execute(
          'INSERT INTO Notifications (UserID, Title, Message, Type) VALUES (?, ?, ?, ?)',
        ...
        return res.status(402).json({
          ...
          transactionRef, status: 'Failed', mode: 'simulation'
        });
      }
    } catch (txErr) {
      await conn.rollback();
